...
namespace WindowsFormsFlower
{
    public partial class frmBOMReg : WindowsFormsFlower.BaseForm.frmListDetail
    {
        ...
        private void btnItemSave_Click(object sender, EventArgs e)
        {
            ...
            string matId = txtMatID.Text;
            int idx = materialBOMAllList.FindIndex((mat) => mat.MaterialID == matId);
            if (idx >= 0)
            {
                //이미 선택된 재료가 BOM에 추가되는 경우
                materialBOMAllList[idx].MaterialQuantity += (int)nmMatQuantity.Value;
            }
            else
            {
                //신규로 재료를 BOM목록에 추가하는 경우
        ...
        {
            if (dgvBOMMatP.SelectedRows.Count<1)
            {
            ...
            int idx = materialBOMAllList.FindIndex((p) => p.MaterialID == dgvBOMMatP.SelectedRows[0].Cells["MaterialID"].Value.ToString());
